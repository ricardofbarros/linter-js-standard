...
var pickStandard = function (style, dir) {
  return allowUnsafeNewFunction(function () {
    switch (style) {
      ...
      case '@niftyco/standard':
        return requireWithLocalOverride('@niftyco/standard', dir)
      default:
...
function getStyleThroughDevDeps (filePath) {
  ...
  var knownLinters = ['standard', 'semistandard', 'happiness', 'uber-standard', '@niftyco/standard']
  ...
  if (hasKnownLinter) {
    ...
    if (devDeps['@niftyco/standard'] || prodDeps['@niftyco/standard']) {
      return pickStandard('@niftyco/standard', dir)
    }
